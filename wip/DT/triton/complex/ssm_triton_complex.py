...
def complex_chunk_update_fwd_kernel(
    ...
    # Load Carry
    # INFORMATION CRITIQUE: carries est un tenseur complexe (R, I, R, I...)
...
def complex_chunk_final_bwd_kernel(
    ...
    # INFORMATION CRITIQUE: grad_carries est complexe (R I R I)
    offs_carry = pid_batch * (tl.num_programs(1) * tl.num_programs(2)) + \
                 pid_dim * tl.num_programs(2) + pid_chunk
    ...
    if HAS_INIT:
        ...
        # INFORMATION CRITIQUE: Hi_init_ptr pointe déjà sur Imag. +1 décale de 1 float vers la partie Réelle suivante.
...
class ComplexSSMFunction(torch.autograd.Function):
    ...
    def forward(ctx, u, a, h_init, chunk_size):
        ...
        # Allocations "Dense" Float pour les résumés (pour éviter stride complexity)
        L_u_r = torch.zeros((B, D, num_chunks), device=u.device, dtype=torch.float32)
        L_u_i = torch.zeros((B, D, num_chunks), device=u.device, dtype=torch.float32)
        L_a_r = torch.zeros((B, D, num_chunks), device=u.device, dtype=torch.float32)
        L_a_i = torch.zeros((B, D, num_chunks), device=u.device, dtype=torch.float32)
        ...
        # Carries est "Dense Complex", donc on passe aussi stride*2 si on utilise .real/.imag views
        # Mais ici carries est [B, D, Chunks], on peut le passer en dense float pointers comme L_u
        # pour éviter confusion. Cependant, .real/.imag sur un contiguous complex retourne stride=2.
        # Donc on applique la règle stride*2 aussi ici.
    ...
    def backward(ctx, grad_output):
        ...
        L_du_r = torch.zeros((B, D, num_chunks), device=u.device, dtype=torch.float32)
        L_du_i = torch.zeros((B, D, num_chunks), device=u.device, dtype=torch.float32)
        L_ar_rev = torch.zeros((B, D, num_chunks), device=u.device, dtype=torch.float32)
        L_ai_rev = torch.zeros((B, D, num_chunks), device=u.device, dtype=torch.float32)
